  ...
  int a , b , c , d; 
  int caseSwitchFlag = 100 ; 
  float tolerance = 1 ; //0.05;
  double kp = 5;
  double ki = 0.002;
  double kd = 5;
  ...
  void landingCallback(const std_msgs::String::ConstPtr& msg)
  {
    if (msg->data =="startLanding")
      caseSwitch = 1 ; 
    //std::cout << "Switch Case 1 call back" <<  caseSwitch << std::endl ; ; 
  }

  void explorationCallback(const std_msgs::String::ConstPtr& msg)
  {
      if (msg->data =="startMission")
	caseSwitch = 0 ; 
        //std::cout << "Switch Case 0 call back" <<  caseSwitch << std::endl ; ; 
  }


  geometry_msgs::PoseStamped goalPose;
  std_msgs::String msg2State  ; 
  //void goalCallback(const geometry_msgs::PoseStamped::ConstPtr& msg)
 // {
  //  ROS_INFO_STREAM("Received Goal: (" << msg->pose.position.x <<","<< msg->pose.position.y<<","<< msg->pose.position.z<<")");
   // goalPose = *msg ;
 // }
  void centerPointCallback (const sensor_msgs :: RegionOfInterest :: ConstPtr& msg){
  std::cout << "call back  if detected " << msg->x_offset << "   " << msg->y_offset  << "          "<<  std::endl ; 
  ...
  void localPoseCallback(const geometry_msgs :: PoseStamped :: ConstPtr& msg)
  {
    ...
    if (caseSwitch == 0 )
    {
      //std::cout << "go to center " <<  caseSwitch << std::endl ; ; 
    goalPose.pose.position.x = 0;
    goalPose.pose.position.y = 0;
    goalPose.pose.position.z = 10;
    }
    else if (caseSwitch == 1 )
    {   
     // std::cout << "follow the target " <<  caseSwitch << std::endl ; ; 

      srv.request.A = a;
      srv.request.B = b;
      std::cout << "U of the marker" << a  << std::endl ; 
      std::cout << "V of the marker" <<  b  << std::endl ; 

       if (client.call(srv))
	{
	    goalPose.pose.position.x = (long int)srv.response.obj.pose.pose.position.x ; 
	    goalPose.pose.position.y = (long int)srv.response.obj.pose.pose.position.y; 
	    goalPose.pose.position.z = 2 ; 
	    std::cout << "x of the marker" <<  goalPose.pose.position.x << std::endl ; 
            std::cout << "Y of the marker" <<  goalPose.pose.position.y << std::endl ;
	...
	{
        std::cout << " hover 1" <<  caseSwitch << std::endl ; ; 
    ...
    {
          std::cout << " hover 2 " <<  caseSwitch << std::endl ; ; 
...
*/
  ...
  int main(int argc , char **argv)
  {
    ...
    ros::Subscriber landingSub          = nh.subscribe("/startLanding", 1000, landingCallback);
    ros::Subscriber explorationSub      = nh.subscribe("/startExploration", 1000, explorationCallback);
    ros::Publisher explorationPub = nh.advertise <std_msgs::String>("/endExploration", 1);
    ros::Publisher landingPub = nh.advertise <std_msgs::String>("/endLanding", 1);
    ros::Subscriber centerPoint = nh.subscribe("/ch1/marker_bb",1000,centerPointCallback);
    client = nh.serviceClient<kuri_mbzirc_challenge_1_msgs::PES>("position_estimation");
	...
	if(ros::Time::now() - statusUpdate > ros::Duration(1.0))
	{
      ...
      if (caseSwitchFlag == 0 ) 
      {
	msg2State.data = "detected" ; 
	explorationPub.publish(msg2State) ; 
	caseSwitchFlag = 100 ; 
      }
      else if(caseSwitchFlag == 1)
      {
	msg2State.data = "onTopOfMarker";
	  landingPub.publish(msg2State) ; 
	  caseSwitchFlag = 100 ; 
      }
      ros:: spinOnce ();
      loopRate.sleep();
